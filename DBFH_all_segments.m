...
[~, ~, U37_only_even] = render_selected_tiles(segments, [2:2:37]);
[~, I37_only_even]    = pupil_fft2(U37_only_even, fft_res);

fig_unstacked = figure;imagesc((I37_only_even));  axis image  ij ; colorbar; colormap gray;
title('scrambled and unstacked','Color','w');


[~, ~, U37_only_odd] = render_selected_tiles(segments, [1:2:37]);
[~, I37_only_odd]    = pupil_fft2(U37_only_odd, fft_res);

fig_unstacked = figure;imagesc((I37_only_odd));  axis image  ij ; colorbar; colormap gray;
title('scrambled and unstacked','Color','w');
...
%% ---- Overlap Captures using NOMINAL tilts only ----
odd_inds = [1:36:37];
even_inds = [2:36:36];

% Align tile 2 spot to tile 1 spot in UNSTACKED tilt space
[segments_even_onto_previous_odd,tilt_x,tilt_y] = overlap_by_nominal_tilt_unstacked(segments, odd_inds(1:end-1), even_inds,unstack_ratio);
[phi12, M12, U12] = render_selected_tiles(segments_even_onto_previous_odd, [odd_inds(1:end-1) even_inds]);
[~, I_even_onto_previous_odd]     = pupil_fft2(U12, fft_res);

% Align tile 2 spot to tile 1 spot in UNSTACKED tilt space with pi/2
segments_even_onto_previous_odd_pi         = segments_even_onto_previous_odd;
segments_even_onto_previous_odd_pi.pistons(even_inds) = segments_even_onto_previous_odd_pi.pistons(even_inds) + pi/2;
[phi12p, M12p, U12p] = render_selected_tiles(segments_even_onto_previous_odd_pi, [odd_inds(1:end-1) even_inds]);
[~, I_even_onto_previous_odd_pi]      = pupil_fft2(U12p, fft_res);

% Align tile 3 spot to tile 2 spot in UNSTACKED tilt space
[segments_odd_onto_previous_even,tilt_x,tilt_y] = overlap_by_nominal_tilt_unstacked(segments,even_inds, odd_inds(2:end),unstack_ratio);
[phi23, M23, U23] = render_selected_tiles(segments_odd_onto_previous_even, [even_inds odd_inds(2:end)]);
[~, I_odd_onto_previous_even]     = pupil_fft2(U23, fft_res);

% Align tile 3 spot to tile 2 spot in UNSTACKED tilt space with pi/2
segments_odd_onto_previous_even_pi = segments_odd_onto_previous_even;
segments_odd_onto_previous_even_pi.pistons(odd_inds(2:end)) = segments_odd_onto_previous_even_pi.pistons(odd_inds(2:end)) + pi/2;
[phi23p, M23p, U23p] = render_selected_tiles(segments_odd_onto_previous_even_pi, [even_inds odd_inds(2:end)]);
[~, I_odd_onto_previous_even_pi]     = pupil_fft2(U23p, fft_res);
...
nexttile; imagesc(log(I37));  axis image ij off; colormap parula; colorbar; title(['un-stacked']);set(gca,'FontSize',font_size);
nexttile; imagesc(log(I_even_onto_previous_odd)); axis image ij off; colormap parula; colorbar; title('even \rightarrow previous odd');set(gca,'FontSize',font_size);
nexttile; imagesc(log(I_even_onto_previous_odd_pi));axis image ij off; colormap parula; colorbar; title('even \rightarrow previous odd + \pi/2');set(gca,'FontSize',font_size);
nexttile; imagesc(log(I_odd_onto_previous_even)); axis image ij off; colormap parula; colorbar; title('odd \rightarrow previous even');set(gca,'FontSize',font_size);
nexttile; imagesc(log(I_odd_onto_previous_even_pi));axis image ij off; colormap parula; colorbar; title('odd \rightarrow previous even + \pi/2');set(gca,'FontSize',font_size);
% exportgraphics(gcf,'figures\inputs_intensities_to_DBFH.png');

%% define masks around PSFs (in the farfield)

[x_psf,y_psf] = find_unstacked_psfs(I37); % sorted

x_psf_diff = x_psf-x_psf';
y_psf_diff = y_psf-y_psf';
r_psd_diff = sqrt(x_psf_diff.^2+y_psf_diff.^2);
min_dist = min(r_psd_diff(r_psd_diff~=0));

r_hex_mask = round(min(r_psd_diff(r_psd_diff~=0))*1/2);

M = make_hex_masks(x_psf, y_psf, r_hex_mask, size(I37));
for i=1:37 M_edges(:,:,i) = edge(M(:,:,i)); end

max_I37 = max(log(I37(:)));
figure;imagesc(log(I37));hold on;axis ij image;colormap gray;
imagesc(max_I37*sum(M_edges,3),'AlphaData',0.5);
plot(x_psf, y_psf, 'yo', 'MarkerSize', 10, 'LineWidth', 1.5);
text(x_psf, y_psf,num2str([1:37]'),'color','r','FontSize',20);
title(sprintf('Detected %d PSFs', numel(x_psf)));



%% prepare data for DBH

for ind_pairing = 2:2:37-1

    this_even = ind_pairing;
    this_odd = this_even - 1;

    this_mask_even = M(:,:,this_even); % farfield masks
    this_mask_odd = M(:,:,this_odd);

    this_dtilt_even = [segments_even_onto_previous_odd.tilt_x(this_even) - segments.tilt_x(this_even), ...
                       segments_even_onto_previous_odd.tilt_y(this_even) - segments.tilt_y(this_even)];
    this_dtilt_odd = [0,0];  % odd tiles unchanged

    Rpx = segments.meta.seg_flat_diam_px / sqrt(3);

    % this_I1 = I37.*this_mask_odd; % this is a mask in the near field!
    % this_I2 = I37.*this_mask_even;
    % this_I2_onto_I1 = I_even_onto_previous_odd.*this_mask_odd;
    % this_I2_onto_I1_pi = I_even_onto_previous_odd_pi.*this_mask_odd;

    % Build |A|, |B|, S on the *centered* grid, then unshift to DFT layout
    % prep = dbh_prepare_from_four_centered(this_I1, this_I2, this_I2_onto_I1, this_I2_onto_I1_pi, this_dtilt_odd, this_dtilt_even, fft_res, Rpx);
    prep = dbh_prepare_from_four_centered(I37, I37, I_even_onto_previous_odd, I_even_onto_previous_odd_pi, this_dtilt_odd, this_dtilt_even, fft_res, Rpx);
    prep.magA = prep.magA.*this_mask_odd;
    prep.magB = prep.magB.*this_mask_odd;
    prep.S = prep.S.*this_mask_odd;
    ...
    [~, CS_this_odd, ~] = render_selected_tiles(segments, this_odd);
    [~, CS_this_even, ~] = render_selected_tiles(segments, this_even);

    suppA = center_padcrop(CS_this_odd, size(magA), false); % mask in the near field!
    suppB = center_padcrop(CS_this_even, size(magA), false);

    % figure;imagesc(suppA-suppB)
    ...
    phi1_0  = rm_piston_phi(phi37,  suppA);
    phi2_0  = rm_piston_phi(phi12.*suppB,  suppB);
    ...
    if 1
        ...
        title(['Tile  ',num2str(this_odd),'  – original']);set(gca,'FontSize',font_size);
        ax2 = nexttile; imagesc(phi1r_0.*nan_mask_tile1); axis image ij off; clim(cl1); colorbar;
        title(['Tile ',num2str(this_odd),' – recovered']);set(gca,'FontSize',font_size);
        ax3 = nexttile; imagesc(d1.*nan_mask_tile1);      axis image ij off;clim(cl2);colorbar;
        title(sprintf('Tile %d – diff (RMS=%.3g)',this_odd,rms1));set(gca,'FontSize',font_size);
        ax4 = nexttile; imagesc(phi2_0.*nan_mask_tile2);  axis image ij off; clim(cl1); colorbar;
        title(['Tile  ',num2str(this_even),'  – original']);set(gca,'FontSize',font_size);
        ax5 = nexttile; imagesc(phi2r_0.*nan_mask_tile2); axis image ij off; clim(cl1); colorbar;
        title(['Tile  ',num2str(this_even),'  – recovered']);set(gca,'FontSize',font_size);
        ax6 = nexttile; imagesc(d2.*nan_mask_tile2);      axis image ij off; clim(cl2);colorbar;
        title(sprintf('Tile %d – diff (RMS=%.3g)',this_even,rms2));set(gca,'FontSize',font_size);
