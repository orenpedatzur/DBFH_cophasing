%% DBFH_all_segments_simultaneously.m
...
% [phi_odd, M_odd, U_odd] = render_selected_tiles(segments, 1);

[~, I_odd]    = pupil_fft2(U_odd, fft_res);

figure;tiledlayout(1,4, "TileSpacing", "compact", "Padding", "tight"); % Adjust TileSpacing and Padding
ax1 = nexttile; imagesc(log(I_odd));  axis image  ij ; colorbar; colormap gray;
title('only odd');

% --- only even tiles ---

[phi_even, M_even, U_even] = render_selected_tiles(segments, [2:2:37]);
% [phi_even, M_even, U_even] = render_selected_tiles(segments, 2);

[~, I_even]    = pupil_fft2(U_even, fft_res);

ax2 = nexttile; imagesc(log(I_even));  axis image  ij ; colorbar; colormap gray;
title('only even');

% --- odd+even ---

[phi37, ~, U37] = render_selected_tiles(segments, [1:37]);
% [phi37, ~, U37] = render_selected_tiles(segments, [1:2]);

[~, I37]    = pupil_fft2(U37, fft_res);

ax3 = nexttile; imagesc(log(I37));  axis image  ij ; colorbar; colormap gray;
title('odd+even');
...
% [phi37p, ~, U37p] = render_selected_tiles(segments_pi, [1,2]);
[~, I37p]    = pupil_fft2(U37p, fft_res);

ax4 = nexttile; imagesc(log(I37p));  axis image  ij ; colorbar; colormap gray;
title('odd+even(+pi/2)');
